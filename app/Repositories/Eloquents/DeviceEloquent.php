...
class DeviceEloquent implements Repository
{
    ...
    function getReceiverToken($receiver_id)
    {
        ...
        $token_android = $this->model->where('user_id', $receiver_id)->where('type', 'android')->pluck('device_token')->toArray();
        $token_ios = $this->model->where('user_id', $receiver_id)->where('type', 'ios')->pluck('device_token')->toArray();
